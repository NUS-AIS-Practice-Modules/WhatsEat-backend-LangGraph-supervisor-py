...
def test_route_agent():
    ...
    sample_locations = json.dumps({
        "origin": {"lat": 1.3521, "lng": 103.8198, "name": "Singapore"},
        "destinations": [
            {"lat": 1.3048, "lng": 103.8318, "name": "ION Orchard"},
            {"lat": 1.2997, "lng": 103.8479, "name": "Marina Bay Sands"}
        ]
    })
    
    test_cases = [
        {
            "name": "Generate route map HTML",
            "input": {
                "messages": [
                    {"role": "user", "content": f"Generate route map for: {sample_locations}"}
                ]
            },
...
def main():
    ...
    try:
        all_results.append(test_route_agent())
    except Exception as e:
        print(f"Failed to test Route Agent: {e}")
